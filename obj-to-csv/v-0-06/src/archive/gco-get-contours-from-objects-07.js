...
GCO.removeShortSegments = function () {
	...
	const arr1 = vertexArrays[0];
	//const v1 = arr1[arr1.length - 1];
	//console.log('v1', v1);
...
GCO.joinAdjacentSegments = function () {
	...
	const geometry = new THREE.BoxGeometry(0.3, 0.3, 0.9);
	...
	for (let i = 0; i < GCO.contourLines.children.length; i++) {
		...
		for (let j = 0; j < lines.children.length; j++) {
			...
			console.log('extra line verts', line.geometry.vertices.length);
	...
	console.log('extra line count', count);
};
